...
| `wuguang-pocket-2.1.6.apk` | 安装包本体（当前版） |

## 以后发新版，只需要换两个文件

1. 把新的 `wuguang-pocket-2.1.6.apk` 放进来（旧的可以留着，也可以删）
2. 改 `android.json`：
...
不用靠人记得 —— `outputs/_verify-index-page-v216.cjs` 的 **D 态**会拿 `android.json`
和兜底逐条比，对不上就报红。
